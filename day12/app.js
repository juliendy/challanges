...
function search() {
    ...
    fetch(
        `https://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=5&appid=${weather.key}`
    )
        .then((res) => res.json())
        ...
        .then(() => {
            for (let i = 0; i < cityInformations.length; i++) {
                ...
                <h1 class="city">${
                    cityInformations[i].name
                }</h1>                
                ${
                    cityInformations[i].state
                        ? `<h1 class="state">|${cityInformations[i].state}|</h1><br>`
                        : "<br>"
                }               
...
cityOptions.addEventListener("click", (e) => {
    ...
    currentCity.city =
        selectedOption.parentNode.querySelector(".city").textContent;
    currentCity.lat =
        selectedOption.parentNode.querySelector(".lat").textContent;
    currentCity.lon =
        selectedOption.parentNode.querySelector(".lon").textContent;
    currentCity.state = selectedOption.parentNode.querySelector(".state")
        ? selectedOption.parentNode.querySelector(".state").textContent
        : "";
    ...
    fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${currentCity.lat}&lon=${currentCity.lon}&appid=${weather.key}&units=metric`
    )
        ...
        .then(() => {
            ...
            <h1 class="date">${newDaysArr[0].dt_txt.split(" ")[0]}</h1>`;
                for (let i = 0; i < 9; i++) {
                    ...
                    dayForecast.innerHTML += `<h1 class="hourly"><span class="time">${
                        newDaysArr[i].dt_txt.split(" ")[1]
                    }</span>${newDaysArr[i].main.temp_min}°C ${pickIcon(
                        newDaysArr[i].weather[0].icon
                    )}     <span class="humidityy">${
                        forecast.list[i].main.humidity
                    }%</span> <i class="fa-solid fa-droplet"></i></h1>`;
                }
                document
            ...
            const firstDayMax = [];
            document.querySelector(".day0").addEventListener("click", () => {
                getDay1();
            });
