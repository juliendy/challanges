...
async function loadSite() {
    ...
    for (i = 0; i < trendingMovies.results.length; i++) {
        ...
        <div class="movie-card">
            <img class="imagee" src=${
                img_path + trendingMovies.results[i].poster_path
            }></img>
            <h1 class="title">${trendingMovies.results[i].title}</h1>
            <h3 class="rating">${
                trendingMovies.results[i].vote_average
            } <i class="fa-solid fa-star"></i></h3>
            <h3 class="id" style="display: none;">${
                trendingMovies.results[i].id
            }</h3>
...
async function loadByGenre(genreID) {
    ...
    const res = await fetch(
        `https://api.themoviedb.org/3/discover/movie?with_genres=${genreID}&sort_by=revenue.desc&api_key=67cf32bdad212a7cc907563a23e39342`
    );
    ...
    for (i = 0; i < moviesByGenre.results.length; i++) {
        ...
        <div class="movie-card">
            <img class="imagee" src=${
                img_path + moviesByGenre.results[i].poster_path
            }></img>
            <h1 class="title">${moviesByGenre.results[i].title}</h1>
            <h3 class="rating">${
                moviesByGenre.results[i].vote_average
            } <i class="fa-solid fa-star"></i></h3>
            <h3 class="id" style="display: none;">${
                moviesByGenre.results[i].id
            }</h3>            
        </div>`;
    }
    allMovies = document.querySelectorAll(".movie-card");
...
async function displayMovieDetails(movieID) {
            ...
            <div class="movie-info">
                <div class="infoo">
                    ...
                    <div class="info">
                        ...
                        <h2 class="date">Relase date: ${
                            movieInfo.release_date
                        }</h2>
                        <div class="vote">
                            <h2 class="average-vote">rating: ${movieInfo.vote_average.toFixed(
                                1
                            )} <i class="fa-solid fa-star"></i></h2>
                            <h2 class="vote-count">all votes: ${
                ...
                <a href="${
                    movieInfo.homepage
                }" class="link" target="_blank">OFFICIAL PAGE</a>
                ...
                <div class="reccomendations">
                    ...
                    <h1 class="rec-title">People who watched "${
                        movieInfo.title
                    }" also watch:</h1>
    ...
    if (reccomendations.results.length > 0) {
        ...
        <div class="movie-card movie-rec">
            ...
            <img class="img-rec imagee" src=${
                img_path + reccomendations.results[i].poster_path
            } alt="Poster of ${reccomendations.results[i].title} "></img>
            <h3 class="id" style="display: none;">${
                reccomendations.results[i].id
            }</h3>
...
async function search(value) {
    ...
    for (i = 0; i < 5; i++) {
        searchRes.innerHTML += `<div class="movie-result movie-card">
                    <img class="img-search" src="${
                        img_path + searchResults.results[i].poster_path
                    }"></img>
                    
                    <div class="search-item-text">
                        <h3 class="title-search">${
                            searchResults.results[i].title
                        }</h3>
                        <h4 class="desc-search">${searchResults.results[
                            i
                        ].overview.slice(0, 180)}...</h4>
                        <h3 class="id" style="display: none;">${
                            searchResults.results[i].id
                        }</h3>
                    </div>
                </div>`;
    }
...
async function searchPageFun(value) {
    ...
    for (i = 0; i < searchResults.results.length; i++) {
        searchGrid.innerHTML += `<div class="movie-card">
            ...
            <h3 class="rating">${
                searchResults.results[i].vote_average
            } <i class="fa-solid fa-star"></i></h3>
            <h3 class="id" style="display: none;">${
                searchResults.results[i].id
            }</h3>
